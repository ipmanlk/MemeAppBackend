...
const getAll = (id) => {
    return new Promise((resolve, reject) => {
        if (id == null) {
            sql = `SELECT id, url, date, likes FROM memes ORDER BY id DESC LIMIT 10`;
        } else {
            sql = `SELECT id, url, date, likes FROM memes WHERE id < ${id} ORDER BY id DESC LIMIT 10`;
        }
        db.all(sql, [], (err, rows) => {
            if (err) {
                reject(err);
...
const getBest = () => {
    return new Promise((resolve, reject) => {
        sql = "SELECT id, url, date, likes FROM memes ORDER BY likes DESC LIMIT 10";
        db.all(sql, [], (err, rows) => {
            if (err) {
                reject(err);
            }
